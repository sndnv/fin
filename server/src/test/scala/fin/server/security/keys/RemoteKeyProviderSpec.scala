...
class RemoteKeyProviderSpec extends UnitSpec with BeforeAndAfterAll {
  "An RemoteKeyProvider" should "provide keys from a JWKs endpoint" in {
    val endpoint = new MockIdentityEndpoint(port = ports.dequeue())
    endpoint.start()
  ...
  it should "support custom connection contexts" in {
    val config: Config = ConfigFactory.load().getConfig("fin.test.security.tls")

    val serverContext = EndpointContext(
      config = EndpointContext.Config(config.getConfig("context-server-jks"))
    )
    ...
    val endpoint = new MockIdentityEndpoint(
      port = ports.dequeue(),
