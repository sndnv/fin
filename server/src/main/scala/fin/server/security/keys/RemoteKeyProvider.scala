...
final class RemoteKeyProvider(
  ...
  private val cache = new ConcurrentHashMap[String, Key]()
          ...
          .sequence(
            jwks.map {
              case Right(jwk) =>
                ...
                val _ = cache.put(jwk.id, jwk.key)
...
object RemoteKeyProvider {
  def apply(
    ...
    refreshRetryInterval: FiniteDuration,
    issuer: String
  )(implicit system: ActorSystem[Nothing]): RemoteKeyProvider =
    ...
    http
      ...
      .flatMap { case HttpResponse(status, _, entity, _) =>
        status match {
          ...
          case _ =>
            entity.discardBytes().future().flatMap { _ =>
